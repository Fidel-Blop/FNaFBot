...
const handler = async (m, { conn, text, usedPrefix, command }) => {
    try {
        if (!text.trim()) 
            return conn.reply(m.chat, `👁️‍🗨️ Atención: Ingresa el nombre de la música que deseas rastrear en las cámaras...`, m)
        ...
        if (!result) throw '⚡ ALERTA: No se encontraron resultados en las cámaras de Youtube.'

        const { title, thumbnail, timestamp, views, ago, url, author, seconds } = result
        if (seconds > 1800) throw '⚠ El video supera el límite de duración (10 minutos).'

        const vistas = formatViews(views)
        const info = `🎥 乂 VIGILANCIA YOUTUBE 乂 🎥\n\n> ❑ Canal » *${author.name}*\n> ♡ Vistas » *${vistas}*\n> ✧︎ Duración » *${timestamp}*\n> ☁︎ Publicado » *${ago}*\n> ➪ Link » ${url}\n\n🔔 Mantén las luces encendidas y las puertas cerradas mientras se reproduce...`
        ...
        if (['play', 'yta', 'ytmp3', 'playaudio'].includes(command)) {
            ...
            if (!audio?.url) throw '⚠ No se pudo obtener el audio desde los sistemas de vigilancia.'
            ...
            if (!video?.url) throw '⚠ No se pudo obtener el video desde los sistemas de vigilancia.'
            m.reply(`> ❀ *Vídeo procesado. Servidor:* \`${video.api}\``)
            await conn.sendFile(m.chat, video.url, `${title}.mp4`, `> ❀ ${title}\n🔔 Mantén las luces encendidas mientras lo visualizas.`, m)
            await m.react('✅')
        }
    } catch (e) {
        await m.react('❌')
        return conn.reply(m.chat, typeof e === 'string' ? e : `⚠︎ ERROR en el sistema de cámaras.\n> Usa *${usedPrefix}report* para alertar al equipo de mantenimiento.\n\n${e.message}`, m)
    }
}
