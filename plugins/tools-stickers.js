...
const fetchStickerVideo = async (text) => {
  ...
  if (!response.data) throw new Error('Music Man no pudo obtener el video del sticker.')
  return response.data
}
  ...
  try {
    ...
    switch (command) {
      case 'brat': {
        ...
        if (!text) return conn.sendMessage(m.chat, { text: '❀ Music Man dice: Por favor, responde a un mensaje o ingresa un texto para crear el Sticker.' }, { quoted: m })
        await m.react('🕒')
        const buffer = await fetchSticker(text)
        if (!buffer) throw new Error('Music Man no pudo generar el sticker.')
        const stiker = await sticker(buffer, false, texto1, texto2)
        await conn.sendFile(m.chat, stiker, 'sticker.webp', '🎭 Music Man dice: ¡Sticker creado con éxito!', m)
        await m.react('✔️')
        break
      }
      case 'bratv': {
        ...
        if (!videoBuffer) throw new Error('Music Man no pudo generar el sticker animado.')
      ...
      case 'emojimix': {
        if (!args[0]) return m.reply(`❀ Music Man dice: Ingresa 2 emojis para combinar.\n> Ejemplo: *${usedPrefix + command}* 👻+👀`)
        ...
        if (!res.results || res.results.length === 0) throw new Error('Music Man no encontró stickers para esos emojis.')
      ...
      case 'qc': {
        ...
        if (!textFinal) return conn.reply(m.chat, `❀ Music Man dice: Ingresa un texto para crear el sticker.`, m)
        ...
        if (frase.length > 30) return await m.react('✖️'), conn.reply(m.chat, `ꕥ Music Man dice: El texto no puede tener más de 30 caracteres.`, m)
        await m.react('🕒')
      ...
      case 'take': case 'wm': {
        if (!m.quoted) return m.reply(`❀ Music Man dice: Responde a un sticker con el comando *${usedPrefix + command}* seguido del nuevo nombre.\n> Ejemplo: *${usedPrefix + command}* NuevoNombre`)
        await m.react('🕒')
        const stickerData = await m.quoted.download()
        if (!stickerData) return await m.react('✖️'), m.reply('Music Man dice: No se pudo descargar el sticker.', m)
    ...
    await m.react('✖️')
    conn.sendMessage(m.chat, { text: `⚠︎ Music Man encontró un problema.\n> Usa *${usedPrefix}report* para informarlo\n\n${e.message}` }, { quoted: m })
  }
}
